...
import { FaCheck, FaTrash, FaMapMarkerAlt, FaPhoneAlt, FaCalendarAlt, FaShoppingBag } from 'react-icons/fa';
          ...
          <Col key={pedido._id}>
            <Card className="h-100 shadow border-0" style={{ background: '#25332f' }}>
              <Card.Header className="bg-transparent border-0 d-flex justify-content-between align-items-center mt-2">
                 ...
                 <Badge bg={obtenerColorEstado(estadoActual)}>
              ...
              <Card.Body className="text-white pt-0">
                ...
                <div className="mb-3 bg-secondary bg-opacity-25 p-2 rounded">
                  <small className="text-white-50 d-block mb-1"><FaShoppingBag className="me-1"/> Resumen:</small>
                  <ul className="list-unstyled mb-0 small text-light ps-2" style={{maxHeight: '80px', overflowY: 'auto'}}>
                ...
                <div className="d-flex justify-content-between align-items-end mb-3">
                    <div className="small text-white-50">
                        <div className="mb-1"><FaMapMarkerAlt className="me-1"/> {pedido.direccion}</div>
                        <div className="mb-1"><FaPhoneAlt className="me-1"/> {pedido.telefono}</div>
                        <div><FaCalendarAlt className="me-1"/> {new Date(pedido.createdAt).toLocaleDateString()}</div>
                    </div>
              ...
              <Card.Footer className="border-top-0 bg-transparent pb-3 pt-0 d-flex justify-content-between">
                {tieneCambios ? (
                  <Button variant="success" size="sm" className="w-100 me-2" onClick={() => onGuardar(pedido._id)}>
