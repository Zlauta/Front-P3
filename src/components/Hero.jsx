...
const Hero = () => {
  ...
  useEffect(() => {
    const intervalo = setInterval(() => {
      setIndiceActual(
        (indiceAnterior) => (indiceAnterior + 1) % imagenesHero.length
      );
    }, DURACION_SLIDE);
    ...
    <section className="hero-seccion">
      ...
      {imagenesHero.map((urlImagen, indice) => (
        <div
          ...
          className={`hero-imagen-fondo ${
            indice === indiceActual ? "hero-activa" : ""
          }`}
      ...
      <div className="hero-contenido">
        ...
        <p className="hero-subtitulo">
          Descubre una experiencia gastronómica inolvidable donde la tradición
