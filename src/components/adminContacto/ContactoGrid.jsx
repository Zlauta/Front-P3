...
import { FaCheck, FaTrash, FaPhone, FaEnvelope, FaCalendarAlt, FaCommentDots, FaReply } from 'react-icons/fa';

// Recibimos nueva prop: onResponder
export default function ContactoGrid({ contactos, ediciones, onEstadoChange, onGuardar, onEliminar, onResponder }) {
            ...
            <Card className="h-100 shadow border-0" style={{ background: '#25332f' }}>
              <Card.Body className="text-white d-flex flex-column">
                ...
                <div className="small text-white-50 mb-3">
                    <div className="d-flex align-items-center mb-1 text-truncate" title={contacto.email}>
                ...
                <div className="bg-secondary bg-opacity-25 p-3 rounded mb-3 flex-grow-1">
                     <p className="small mb-0 fst-italic text-light" style={{maxHeight: '100px', overflowY: 'auto'}}>
                        "{contacto.mensaje}"
                    </p>
                </div>
              ...
              <Card.Footer className="border-top-0 bg-transparent pb-3 pt-0">
                <div className="d-flex gap-2 mb-2">
                    ...
                    >
                        <FaReply className="me-2"/>Responder
                    </Button>
                </div>
                <div className="d-flex justify-content-between">
                    {tieneCambios ? (
                    <Button variant="success" size="sm" className="w-100 me-2" onClick={() => onGuardar(contacto._id)}>
                        <FaCheck className="me-2"/> Guardar
                    </Button>
                    ) : <div className="w-100 me-2"></div>}

                    <Button variant="outline-danger" size="sm" onClick={() => onEliminar(contacto._id)}>
                         <FaTrash />
                    </Button>
                </div>
              </Card.Footer>
