...
const ModalCreateResenia = ({ show, handleClose, updateList }) => {
  ...
  // Estilos (Tus colores)
  const styles = {
    modalContent: { backgroundColor: '#254630', color: '#fff', border: '1px solid #1aaf4b' },
    // Nota: El borde se sobrescribe automáticamente a rojo si hay error gracias a 'isInvalid'
    input: { backgroundColor: '#122117', color: '#fff', border: '1px solid #1aaf4b' },
    btnPrimary: { backgroundColor: '#1aaf4b', borderColor: '#1aaf4b', color: '#fff', fontWeight: 'bold' },
                ...
                return (
                  <label key={index}>
                    <input 
                      ...
                      name="calificacion" 
                      value={ratingValue} 
                      onClick={() => { setRating(ratingValue); setValue('calificacion', ratingValue); }}
                      style={{ display: 'none' }}
                    />
                    <FaStar 
                      size={35} 
...
export default ModalCreateResenia;