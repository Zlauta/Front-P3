...
const Header = () => {
  ...
  const commonLinks = (
    <>
      ...
      <Nav.Link href="/#sobre-nosotros" className="scroll" onClick={() => setExpanded(false)}>
        Sobre Nosotros
      </Nav.Link>
      <Nav.Link href="/#resenias" className="scroll" onClick={() => setExpanded(false)}>
        Reseñas
      </Nav.Link>
          ...
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className="ms-auto mb-2 mb-lg-0 align-items-lg-center">
              {isActiveUser && user.rol === 'cliente' && (
                <>
                  {commonLinks}
                  <Nav.Link as={Link} to="/reservas" onClick={() => setExpanded(false)}>
                    Reservas
                  </Nav.Link>
              ...
              {isActiveUser && user.rol === 'admin' && <Navigate to="/admin" replace />}
              {(!isActiveUser || (user.rol !== 'cliente' && user.rol !== 'admin')) && (
                <>
                  {commonLinks}
                  <Nav.Link
                    onClick={() => {
